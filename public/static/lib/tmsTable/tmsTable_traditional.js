...
jQuery.tmsTable = function(urlBusca, limitable, ordering) {
    ...
    if (ajaxBusca !== '#') { // verifica se o parâmetro de url do ajax de busca foi setado
        (function() { // função Self-Invoking para colocar imagem de ordenação nos th's da tabela, div de paginação e select de ítens por página
            ...
            for (var i = 0; i < colunas; i++) { // coloca imagens de ordenção nos th's da tabela                                 
                $('[id^="oTms_"]').eq(i).append('<img src="http://omundolenovo.com.br/teste_novo_layout/public/js/tmsTable/img/sort_both.png">'); // coloca imagem em todos os th's                    
            }
            ...
            $('#box_tmsTable').prepend('<select id="iTms" class="iTms">' + selectOptions + '</select>'); // insere o select de ítens por página            
        }());
    }
    ...
    function ordenarBotoesPaginacao(totalRegistrosAtual) { // faz cálculos para implementar os botões de paginação            
        ...
        for (var i = 1; i < total; i++) { // insere botões de paginação na div de botões de paginação                
            ...
            ((paginacao > 1) ? $('#pTms_page' + paginacao).addClass('paginacao_ativa') : $('#pTms_page1').addClass('paginacao_ativa'));
        }
    }
    ...
    function alterarImagem(id, direc) { // altera imagem de ordenação            
        $('[id^="oTms_"] img').attr('src', 'http://omundolenovo.com.br/teste_novo_layout/public/js/tmsTable/img/sort_both.png'); // tira imagem de ordenação das colunas                    
        $('#' + id + ' img').attr('src', 'http://omundolenovo.com.br/teste_novo_layout/public/js/tmsTable/img/sort_' + direc + '.png'); // coloca imagem específica 
    }
